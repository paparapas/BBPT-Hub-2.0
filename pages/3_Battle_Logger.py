...
# AJUSTADO: Recebe também os combos usados e envia para a DB
def archive_match_to_supabase(evento, battle_id, p1, p2, p1_combo, p2_combo, score1, score2, log_list):
    try:
...
# INTERFACE
# ==========================================
tabs = st.tabs(["🤺 Emparceiramento", "⚔️ Combate em Curso", "📋 Histórico Temporário", "📥 Exportar ZeroLogs"])
            ...
            with col_new:
                if st.button("✅ Confirmar o Resultado e Voltar ao Lobby do Evento", use_container_width=True, type="primary"):
                    if 'arquivado' not in st.session_state:
                        # Envia os dados para a BD (incluindo combos)
                        archive_match_to_supabase(
                            ...
                            c_p1 if c_p1 else "N/A",  # Combo P1
                            c_p2 if c_p2 else "N/A",  # Combo P2
                    ...
                    # CORREÇÃO DA QUEDA DE SESSÃO: Limpa cirurgicamente as chaves da batalha, mantém o login.
...
with tabs[3]:
    st.subheader("📥 Extração Oficial (ZeroLogs)")
    ...
    if not logs_data:
        st.info("Ainda não existem registos de ZeroLogs na Base de Dados do Supabase.")
    else:
        ...
        nome_ficheiro = f"ZeroLogs_{filtro_evento.replace(' ', '_')}.csv" if filtro_evento != "Todos os Torneios" else "ZeroLogs_Todos_Os_Torneios.csv"
