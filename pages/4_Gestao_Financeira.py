...
# ⚡ OTIMIZAÇÃO 1: CACHE NAS LEITURAS DE REGISTOS ⚡
@st.cache_data(ttl=600)
...
# ⚡ OTIMIZAÇÃO 2: FRAGMENTO PARA ATUALIZAR VALORES SEM RECARREGAR A PÁGINA ⚡
@st.fragment
def painel_configuracoes_valores(active_tourney):
...
painel_configuracoes_valores(active_tourney)

current_entry_final = float(active_tourney.get("entry_fee", 2.0) or 2.0)
current_quota_final = float(active_tourney.get("league_quota", 1.0) or 1.0)

st.info(f"🏆 A gerir o Torneio: **{active_tourney['name']}** |  Inscrição: **{current_entry_final}€** |  Quota: **{current_quota_final}€**")

# ⚡ OTIMIZAÇÃO 3: FRAGMENTO GIGANTE PARA A GRELHA E KPIS ⚡
# Agora podes clicar nas checkboxes à vontade que a página não pisca!
@st.fragment
def painel_interativo_grelha(active_tourney, new_entry, new_quota):
            ...
            with st.spinner("A sincronizar dados..."):
                ...
                if updates:
                    try:
                        ...
                        fetch_registrations.clear() # Limpa a cache forçando leitura de dados frescos
                        st.success(f"✅ {len(updates)} registo(s) financeiro(s) atualizado(s)!")
                        st.rerun() # Atualiza os KPIs finais
...
painel_interativo_grelha(active_tourney, current_entry_final, current_quota_final)